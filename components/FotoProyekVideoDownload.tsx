...
export function FotoProyekVideoDownload({ projectId }: { projectId: string }) {
  ...
  const href = useMemo(() => {
    ...
    return `/api/foto-proyek/video-download?${q.toString()}`;
  }, [projectId, from, to]);

  return (
    <div className="space-y-3 rounded-xl border border-[var(--line-soft)] bg-[#fffcf7] p-3">
      <p className="text-sm font-medium text-[var(--ink)]">Download video</p>
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Dari tanggal" htmlFor="vdl-from">
          <input
            id="vdl-from"
            type="date"
        ...
        <Field label="Sampai tanggal" htmlFor="vdl-to">
          <input
            id="vdl-to"
            type="date"
            className={inputClass}
